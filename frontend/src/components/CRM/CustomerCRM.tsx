...
export const CustomerCRM = ({ currentUser }: Props) => {
  ...
  const [activeTab, setActiveTab] = useState<'daftar' | 'retensi'>('daftar');
  ...
  return (
    <div className="crm-container">
      <div className="tab-nav" style={{ marginBottom: '1.5rem', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--glass-border)', padding: '0.4rem', borderRadius: '14px', width: 'fit-content', display: 'flex', gap: '0.25rem' }}>
        <button 
          onClick={() => setActiveTab('daftar')}
          style={{ 
            padding: '0.6rem 1.25rem', borderRadius: '10px', fontSize: '0.85rem', fontWeight: 700,
            background: activeTab === 'daftar' ? 'var(--primary)' : 'transparent',
            color: activeTab === 'daftar' ? 'white' : 'var(--text-muted)',
        ...
        <button 
          ...
          style={{ 
            padding: '0.6rem 1.25rem', borderRadius: '10px', fontSize: '0.85rem', fontWeight: 700,
            background: activeTab === 'retensi' ? 'var(--primary)' : 'transparent',
            color: activeTab === 'retensi' ? 'white' : 'var(--text-muted)',
            transition: 'all 0.2s', border: 'none', cursor: 'pointer',
        ...
        >
          Retensi Pelanggan
          {activeTab !== 'retensi' && (
            <span style={{ fontSize: '0.65rem', padding: '0.1rem 0.4rem', borderRadius: '4px', background: 'rgba(255,223,0,0.2)', color: '#ffdf00' }}>
              PRO
            </span>
          )}
        </button>
      ...
      {activeTab === 'retensi' && (
        <CustomerRetention customers={customers} transactions={transactions} />
      )}
