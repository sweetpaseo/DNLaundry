...
// .wrangler/tmp/bundle-BXGExd/checked-fetch.js
...
// .wrangler/tmp/bundle-BXGExd/strip-cf-connecting-ip-header.js
...
var GoTrueClient = class {
  ...
  constructor(options) {
    ...
    this.fetch = resolveFetch3(settings2.fetch);
...
var SupabaseClient = /* @__PURE__ */ __name(class {
  ...
  */
  constructor(supabaseUrl, supabaseKey, options) {
    ...
    this.fetch = fetchWithAuth(supabaseKey, this._getAccessToken.bind(this), settings2.global.fetch);
...
app.get("/", (c) => {
  return c.html(`
    <body style="font-family: sans-serif; padding: 2rem; background: #0f172a; color: white;">
      <h1 style="color: #818cf8;">\u{1F680} Antigravity Laundry API</h1>
...
services.get("/", async (c) => {
  ...
  if (error)
    return c.json({ error: error.message }, 500);
  return c.json(data || [
    { id: "1", name: "Cuci Kering", price: 6e3, unit: "kg", is_active: true },
    { id: "2", name: "Setrika", price: 4e3, unit: "kg", is_active: true },
    { id: "3", name: "Express", price: 1e4, unit: "kg", is_active: true },
    { id: "4", name: "Bedcover", price: 25e3, unit: "pcs", is_active: true }
  ]);
});
...
auth.post("/login", async (c) => {
  ...
  const { data, error } = await supabase.from("laundry_users").select("id, username, name, role, password").eq("username", username).single();
...
auth.put("/change-password", async (c) => {
  ...
  const supabase = getSupabase(c.env);
  const { error } = await supabase.from("laundry_users").update({ password: newPassword }).eq("id", userId);
  if (error)
    return c.json({ error: error.message }, 500);
  return c.json({ success: true });
});
...
// .wrangler/tmp/bundle-BXGExd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-BXGExd/middleware-loader.entry.ts
